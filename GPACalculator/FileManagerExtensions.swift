...
extension FileManager {
    ...
    func save(jsonDict: JsonDictionary, to baseUrl: URL, withId id: String) -> Bool {
        let url = baseUrl.appendingPathComponent("\(id).json")
        do {
            let jsonData = try JSONSerialization.data(withJSONObject: jsonDict, options: .init(rawValue: 0))
            let _ = try jsonData.write(to: url, options: .noFileProtection)
            return true
        } catch {
    ...
    func readJsonDict(withId id: String, at baseUrl: URL) -> JsonDictionary? {
        let url = baseUrl.appendingPathComponent("\(id).json")
    ...
    func deleteJsonDict(withId id: String, at baseUrl: URL) -> Bool {
        let url = baseUrl.appendingPathComponent("\(id).json")
        return deleteFile(atUrl: url)
    }
